...
//        printInfo.printer = NSPrinter(name: "VoidPrinter") ?? <#default value#>
    ...
    func runOpration(
        view printView: ContentPrintView, info printInfo: NSPrintInfo
    ) -> Bool {
        ...
        print(
            runResult
                ? "✅ runResult PDF saved" : "❌ runResult PDF generation failed")
        return runResult
    }
    ...
    func printContentToPDF(
        finalPath: URL, fileURL: URL, docType: NSAttributedString.DocumentType
    ) async -> Bool {
        let result = await MainActor.run { () -> Bool in
            guard
                let attributedText = getAttributedText(
                    fileURL: fileURL, docType: docType)
            else {
                return false
            }
